...
export function getAllUsers(page, limit) {
    return async function getAllUsersThunk(dispatch, getState) {
        try {
            ...
            const { data } = await axios.get(`http://localhost:5000/api/v1/users?page=${page}&limit=${limit}`);
...
export function createUser(userData) {
    return async function createUserThunk(dispatch, getState) {
        try {
            dispatch(setLoading(true));
            const { data } = await axios.post("http://localhost:5000/api/v1/user/new", userData);
            dispatch(setLoading(false));
            if (data.success) {
...
export function updateUser(id, updateData) {
    return async function updateUserThunk(dispatch, getState) {
        try {
            dispatch(setLoading(true));
            const { data } = await axios.put(`http://localhost:5000/api/v1/user/${id}`, updateData)
            dispatch(setLoading(false));
            if (data.success) {
                toast.success(data.message);
            } else {
...
export function deleteUser(id) {
    return async function deleteUserThunk(dispatch, getState) {
        try {
            dispatch(setLoading(true));
            const { data } = await axios.delete(`http://localhost:5000/api/v1/user/${id}`);
            dispatch(setLoading(false));
            if (data.success) {
                toast.success(data.message);
            } else {
